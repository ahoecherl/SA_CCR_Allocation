\documentclass[11pt]{article}
    ...
    % Document parameters
    % Document title
    \title{Exemplary SA-CCR allocation under consideration of a minimum transfer amount}
...
\todo{Replace with central difference to reduce the slight delta between EAD and result without MTA}
in line with equation \todo{reference forward dif eq} with and without
...
The RC is also \(>0\), if \(TH < IM_{calc} < MTA\). Based on the
...
\todo{Reference appropriate section in Results}.
