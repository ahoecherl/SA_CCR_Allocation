...
\begin{table}[htbp]
  ...
  \end{table}%
\todo{Clarify RC under Clearing, need to tidy up subscripts}
...
The collateral that is received at a point in time $t$ can be calculated as
\begin{align}
	\label{eq:C}
	C_t = \begin{cases}
		0  &\text{if } |C_{calc}|<\text{TH}\\
		C_{t-1} &\text{if } |C_{calc}|>\text{TH} \text{ and } |C_{calc}-C_{t-1}| < \text{MTA} \\
