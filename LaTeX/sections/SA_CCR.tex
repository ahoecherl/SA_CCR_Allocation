...
\(C\) is defined according to the \emph{NICA} definition, which in accordance with paragraph 143 of \cite{SACCR}. By making assumption \todo{Could add reasoning to make this assumption} \ref{as:Collateral assumptions}
...
\begin{assumption}\label{as:IM larger than TH and MTA}
		...
		\todo{This assumption is wrong because of the way threshold actually works...}
\end{assumption}
...
\begin{table}[htbp]
  ...
  \end{table}%
\todo{Clarify RC under Clearing, need to tidy up subscripts, 0 for RC und bilateral margin is wrong, clarify what $TH_{IM}$ is.}
...
These are the usual rules in place in margin agreements applicable for the case of bilateral derivatives collateralized with VM and IM and are in line with the minimum requirements by the regulators\todo{Reference these minimum requirements}.
...
The case that is analyzed the most in this thesis is \ref{itm:Bilateral derivatives collateralized with VM and IM}. It is important to note that \(RC\) is always floored at zero in this case and a change in VM or IM then only impacts the \gls{SA-CCR} \gls{EAD} through the use of \(C\) in the multiplier calculation of equation \ref{eq:multiplier}. \todo{This is wrong RC can be larger than 0}
